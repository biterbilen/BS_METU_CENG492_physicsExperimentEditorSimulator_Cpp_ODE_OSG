...
SSliderJointWidget::SSliderJointWidget( QWidget* parent, const char* name, WFlags fl )
        : UiSliderJointWidget( parent, name, fl )
{
    mJoint = 0;
    mNeedsRedraw = true;
}

void SSliderJointWidget::setObject( SJoint * obj )
{
    mJoint = dynamic_cast<SSlider *> ( obj );

    if ( mJoint == 0 )
    {
        std::cout << "FATAL ERROR: cannot convert SJoint* to SSlider*" << std::endl;
        return ;
    }
