...
SHinge2JointWidget::SHinge2JointWidget( QWidget* parent, const char* name, WFlags fl )
        : UiHinge2JointWidget( parent, name, fl )
{
    mJoint = 0;
    mNeedsRedraw = true;
}

void SHinge2JointWidget::setObject( SJoint * obj )
{
    mJoint = dynamic_cast<SHinge2 *> ( obj );

    if ( mJoint == 0 )
    {
        std::cout << "FATAL ERROR: cannot convert SJoint* to SHinge2*" << std::endl;
        return ;
    }
