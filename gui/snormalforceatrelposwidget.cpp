...
SNormalForceAtRelPosWidget::SNormalForceAtRelPosWidget(QWidget* parent, const char* name, WFlags fl)
        : UiNormalForceAtRelPosWidget(parent, name, fl)
{
    mForce = 0;
    mNeedsRedraw = true;
}

void SNormalForceAtRelPosWidget::setForce(SForce * obj)
{
    mForce = dynamic_cast<SForceAtRelPos*> (obj);

    if(mForce == 0)
    {
        std::cout << "FATAL ERROR: cannot convert SForce* to SForceAtRelPos*" << std::endl;
        return;
    }
...
void SNormalForceAtRelPosWidget::reset()
{
    QString str;
...
void SNormalForceAtRelPosWidget::save()
{
    if(mForce.valid())
    {
        osg::Vec3 vec;
